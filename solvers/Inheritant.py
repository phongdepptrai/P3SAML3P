...
def find_inherited_peak() -> Optional[Tuple[int, int, int, Path]]:
    """Return (peak, r_src, R_src, root_dir) from a previous run if available."""
    summary_files = [
            ...
            with summary_path.open("r", encoding="utf-8") as f:
                ...
                for row in reader:
                    try:
                        ...
                        c_val = int(row.get("c", "0"))
                        r_val = int(row.get("r_max", "0"))
                        R_val = int(row.get("R_max", "0"))
                        peak_val = int(float(row.get("peak", "0")))
                    except Exception:
                        continue
                    ...
                    inherit_from_r = (r_val == r_max - 1 and R_val == R_max)
                    inherit_from_R = (r_val == r_max and R_val == R_max - 1)
                    if inherit_from_r or inherit_from_R:
                        candidates.append((peak_val, r_val, R_val, summary_path.parent))
        except Exception:
            continue
...
def run_single_instance(name_param, m_param, c_param, r_max_param, R_max_param):
    ...
    load_power(name)
    generate_vars()
    inheritance = find_inherited_peak()
    inherited_peak = None
    inherited_from = None
    if inheritance is not None:
        inherited_peak, inherited_r, inherited_R, inherited_from = inheritance
        print(f"Inheriting peak {inherited_peak} from r_max={inherited_r}, R_max={inherited_R} ({inherited_from.name})")
    else:
        print("No inherited peak available, solving from scratch")
    print("Staircase 1 3")
    ...
    attempts = 0
    LB = max(W) if W else 0

    if inherited_peak is None:
        ...
        best_peak = inherited_peak
        runtime_first = time.time() - t_start
        add_summary_row(
            status="FEASIBLE_INHERITED",
            peak=best_peak,
            attempts=0,
            runtime_sec=runtime_first,
            base_vars=var_counter,
    ...
    if inherited_peak is not None:
        if U:
            solver.add_clause([-U[-1]])
            print(f"Searching for peak < {inherited_peak}")
        else:
            print("Inherited peak equals lower bound; no tighter search possible.")
    ...
    if best_model is not None:
        ...
        print("No model found; inherited peak could not be improved or verified.")
