...
Mô hình ILP được dùng như baseline chính xác. Các biến quyết định:
\[
X_{j,k} =
\begin{cases}
...
\[
F(j,t)=
\sum_{\tau=\max\{0,t-t_j+1\}}^{\min\{t,H-t_j\}} S_{j,\tau}.
\]
...
&& \forall (i,j)\in E,\ \forall k\in M, \tag{4}\\
\sum_{t\in\mathcal{T}^{j}}S_{j,t} &= 1,
&& \forall j\in O, \tag{5}\\
X_{j,k}-R_{k,r}
&\le \sum_{t\in \mathcal{T}^{j}_{r-1}} S_{j,t},
&& j\in O,\ k\in M,\ r=1,\ldots,r_{\max}, \tag{6}\\
S_{j,t}
&\le \sum_{\tau=0}^{t-t_i}S_{i,\tau}
    +2-X_{i,k}-X_{j,k},
&& (i,j)\in E,\ k\in M,\ t\in\mathcal{T}^{j}, \tag{7}\\
X_{i,k}+X_{j,k}+F(i,t)+F(j,t)
&\le 3,
&& i<j,\ k\in M,\ t\in\mathcal{T}, \tag{8}\\
\sum_{j\in O}\sum_{r=1}^{r_{\max}}
w_jF\big(j,(r-1)c+\tau\big)
&\le W_M,
&& \forall \tau=0,\ldots,c-1, \tag{9}\\
R_{k,r+1}&\le R_{k,r},
&& \forall k\in M,\ r=1,\ldots,r_{\max}-1, \tag{10}\\
...
Trong (4) và (7), ràng buộc chỉ áp dụng cho cung tiên quyết thật $(i,j)\in E$, không áp dụng cho mọi cặp $i<j$. Ngược lại, ràng buộc không chồng lấn (8) áp dụng cho mọi cặp tác vụ.
...
R_{k,r}\in\{0,1\}.
\]
Biến $A_{j,t}$ mô tả trạng thái hoạt động của tác vụ $j$ tại thời điểm ảo $t$.
...
\]
Hai biến biên $Y_{j,m-1}$ và $T_{j,H-t_j}$ không cần xuất hiện trong mô hình, vì sau các ràng buộc ``đúng một lựa chọn'', chúng trở thành các giá trị hiển nhiên.
...
Với $k=1,\ldots,m-2$:
\begin{align}
Y_{j,k-1} &\Rightarrow Y_{j,k}, \tag{13}\\
X_{j,k} &\Rightarrow Y_{j,k}, \tag{14}\\
X_{j,k} &\Rightarrow \neg Y_{j,k-1}, \tag{15}\\
Y_{j,k} &\Rightarrow Y_{j,k-1}\lor X_{j,k}. \tag{16}
\end{align}
...
Với $t=1,\ldots,L_j-1$:
\begin{align}
T_{j,t-1} &\Rightarrow T_{j,t}, \tag{18}\\
S_{j,t} &\Rightarrow T_{j,t}, \tag{19}\\
S_{j,t} &\Rightarrow \neg T_{j,t-1}, \tag{20}\\
T_{j,t} &\Rightarrow T_{j,t-1}\lor S_{j,t}. \tag{21}
\end{align}
...
\Big(T_{j,t-t_j}\lor \neg T_{j,t+t_i-1}\Big).
\]
Sau khi xử lý các trường hợp biên của miền thời gian, điều kiện trên trở thành
\[
...
\tag{30}
\]
Điểm quan trọng là vùng cuối phải được viết như (30). Không thể thay điều kiện hiển nhiên bằng $T_{j,L_j-1}$, bởi khi $j$ bắt đầu đúng tại thời điểm cuối $L_j$, biến $T_{j,L_j-1}$ nhận giá trị sai và ràng buộc mất tác dụng.
...
\tag{32}
\]
Khi $q(j,t)>r_{\max}$, thời điểm bắt đầu $t$ vượt quá khả năng tài nguyên của mọi trạm, nên tổ hợp này bị loại bỏ:
\[
(\neg S_{j,t}\lor \neg X_{j,k}).
\tag{33}
\]
...
\tag{34}
\]
Ràng buộc pseudo-Boolean (34) được mã hóa sang CNF bằng bộ mã hóa PB, ví dụ binmerge.
...
\tag{35}
\]
Mỗi ràng buộc trong (35) là một bất đẳng thức pseudo-Boolean. Khi toàn bộ công thức còn khả thi sau khi thêm (35), ta thu được một lịch có đỉnh công suất không vượt quá $B$.
...
\]
với các giá trị $B<UB$ để cải thiện nghiệm. Khi không còn nghiệm với cận thấp hơn, $UB$ là đỉnh công suất tốt nhất trong phạm vi tìm kiếm.
...
\tag{38}
\]
Khi $\neg U_B$ được thêm vào, do tính đơn điệu (36), mọi $U_\ell$ với $\ell\ge B$ cũng bị buộc sai. Khi đó số hạng phụ trong (37) tăng ít nhất $UB-B$, nên (37) suy ra
\[
\sum_{j\in O} w_j\sum_{q=0}^{r_{\max}-1} A_{j,\tau+qc}
\le B.
\]
...
\]
Khi đó các ràng buộc thứ tự trạm (17) và thứ tự thời gian (28)--(30) được áp dụng cho mọi $(i,j)\in E^*$ thay vì chỉ cung trực tiếp $(i,j)\in E$.
...
\item \textbf{SAT kế thừa}: khởi tạo $UB$ bằng cận tốt nhất từ cấu hình tài nguyên nhỏ hơn theo (40).
